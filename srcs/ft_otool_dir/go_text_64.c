...
/*   Updated: 2018/06/26 22:20:18 by gpouyat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	*go_lc_text_64(t_arch input)
{
	...
	while (++index < input.ncmds)
	{
		if (ifswap32(&input, lc->cmd) == LC_SEGMENT_64)
		{
			...
			if (!ft_strncmp(((struct segment_command_64 *)lc)->segname, SEG_TEXT, 16) || !ft_memcmp(((struct segment_command_64 *)lc)->segname, cmp, 16))
				return (lc);
		}
